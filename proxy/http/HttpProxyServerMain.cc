...
// used to check whether we are ready to start the accept threads
int wait_for_et_net_ready = 0;
 ...
 *  after incrementing the counter to see if we can start the
 *  accept threads (i.e. HttpProxyServer). The logic behind the check
 *  is expalined in Main.cc
 */
void
init_HttpProxyServer(EThread *)
{
  ...
  // Change wait_for_et_net_ready from 0 to 2, succeeding in that means
  // we're here first and shouldn't start the accept_threads since other
  // places might not be ready yet.
  if (num_accept_threads == 0 && check_et_net_num == num_of_net_threads && !ink_atomic_cas(&wait_for_et_net_ready, 0, 2)) {
